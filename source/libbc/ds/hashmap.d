module libbc.ds.hashmap;

import libbc.ds.alloc, libbc.ds.array, std.digest.murmurhash, std.digest.crc;
...
)
{
    ...
    ValueT tryGet(KeyT key, out bool wasFound)
    {
        ...
        if(!ptr)
        {
            ValueT init; // Sometimes the compiles things SomeType.init needs TypeInfo, sometimes it doesn't, so we'll just be safe.
            wasFound = false;
            return init;
        }
